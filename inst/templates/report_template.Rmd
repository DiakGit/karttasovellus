...
```{r kartta, fig.height=6, fig.width=3.5}
...
ggplot(data = dat) +                    
    geom_sf(color = alpha("dim grey", 1/3), fill = NA, size = .3)  +
    theme_minimal(base_family = "PT Sans", base_size = 12) +
    labs(fill = NULL,
         title = NULL,
         subtitle = NULL,
         caption = NULL) +
  geom_sf(data = dat_naapurit, fill = alpha("#3b0083", 1/6), color = alpha("white", 1/3)) +
  geom_sf(data = dat_region, fill = alpha("#3b0083", 1/3), color = alpha("white", 1/3)) +
