...
shinyServer(function(input, output) {
    ...
    output$output_info_controlbar <- renderUI({
        
        if (input$sidebar_menu == "map"){
            tagList(
                # tags$h3("Ohjeita"),
                tags$div(class="alert alert-info", role="alert",
                         tags$h4("Näin käytät sovellusta!"),
                         tags$p(tags$strong("Valitse muuttujaluokka"),
                                "-valikosta voit säädellä haluatko katsoa huono-osaisuuden summatietoja vai tarkemmin kunkin huono
                 -osaisuuden ulottuvuuden osoittimia. Kartan voit tallentaa laitteellesi ",tags$strong("Tallenna kartta"),"-vällehdeltä.
                 ",tags$strong("Valitse muuttuja"),"-valikosta voit valita tarkemman osoittimen. 
                 Kartan yläpuolelta voit valita aluetason: ",tags$i("maakunnat, seutukunnat ja kunnat."),
                                tags$br(),
                 ...
                 Alueprofiilista näet kartalta valitsemasi alueen tiedot kaikista osoittimista. Voit tallentaa sen laitteellesi.",tags$a(href = "https://dialogi.diak.fi/2020/05/13/uusi-karttasivusto-paljastaa-miten-huono-osaisuus-vaihtelee-alueiden-valilla/", "Lue lisää Diakin blogista!"))
                )
            )
        } else {
            tagList(
    ...
    output$rank_tbl <- DT::renderDataTable({
        ...
        names(dat) <- c("rank",input$value_regio_level2,input$value_variable)
            ...
            tagList(
                radioButtons("value_map_format", 
                             ...
                             choiceNames = list("vektorikuva (.pdf)",
                                                "vektorikuva (.svg)",
                                                "bittimappikuva (.png)"),
                             choiceValues = list(".pdf", ".svg", ".png"),
                             inline = TRUE),
                downloadButton("save_map", "Tallenna kartta")
            )
        }
    })
    ...
    output$save_map <- downloadHandler(
        ...
        filename = function() {
            file_name <- glue("map_{janitor::make_clean_names(input$value_variable)}_{tolower(input$value_regio_level2)}{input$value_map_format}")
            return(file_name)
        },
        content = function(file) {
                ...
                value = 0,
                {
                    ...
                    if (input$value_regio_level2 != "Kunnat"){
                        ...
                        p + geom_text(data = dat %>%
                                          sf::st_set_geometry(NULL) %>%
                                          bind_cols(dat %>% sf::st_centroid() %>% sf::st_coordinates() %>% as_tibble()),
                                      aes(label = value, x = X, y = Y), color = "white", family = "PT Sans", alpha = .6, size = 1.5) -> p
                    }
