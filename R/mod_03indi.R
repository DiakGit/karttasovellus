         ...
         tags$div(class = "row",
                  tags$div(class = "col-lg-3 grey-background",
                           ...
                           selectInput(inputId = ns("value_variable"), 
                                       ...
                                       choices = NULL,
                                       selected = NULL, #levels(opt_indicator2)[1],
                                       multiple = FALSE),
...
mod_03indi_server <- function(id){
  moduleServer( id, function(input, output, session){
    ...
    react_regio_level <- reactive({
      ...
      return(opt_indicator)
    })
    
    
    observeEvent(react_regio_level(), {
      freezeReactiveValue(input, "value_regio_level")
      opt_indicator <- react_regio_level()
      # updateSelectInput(inputId = "value_regio_level", choices = opt_indicator, selected = opt_indicator[1])
      updateRadioButtons(inputId = "value_regio_level", choices = opt_indicator, selected = opt_indicator[1])
    })

    react_regio_selected <- reactive({
    ...
    observeEvent(react_regio_show_mode(), {
      freezeReactiveValue(input, "value_regio_show_mode")
      opt_indicator <- react_regio_show_mode()
